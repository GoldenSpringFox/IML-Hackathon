...
os.environ["OMP_NUM_THREADS"] = "all"
...
def _build_training_grid(df: pd.DataFrame) -> pd.DataFrame:
    ...
    demand = df.groupby(["city", "sid", "hour_ts"]).size().reset_index(name="demand")
    ...
    city_bounds = (
        demand.groupby("city")["hour_ts"].agg(c_min="min", c_max="max").reset_index()
    )
...
def _compute_lookup_artifacts(grid: pd.DataFrame, global_mean: float) -> dict:
    ...
    city_weekday_hour_stats = {
        f"{c}_{wd}_{h}": v
        for (c, wd, h), v in tmp.groupby(["city", "weekday", "hour"])["demand"]
        .mean()
        .items()
    }
    ...
    station_weekday_hour_stats = {
        f"{s}_{wd}_{h}": v
        for (s, wd, h), v in tmp.groupby(["sk", "weekday", "hour"])["demand"]
        .mean()
        .items()
    }
...
def main() -> None:
    ...
    print(
        f"  {len(grid):,} station-hour rows | demand mean={grid['demand'].mean():.3f}"
    )
    ...
    # ── שלב א': אימון מסווג (האם יש ביקוש > 0) ──────────────────────────────────
    y_clf = (y > 0).astype(int)

    print("Training Classifier (is_demand > 0, early stopping) ...")
    ...
    clf_val.fit(X, y_clf)
    clf_best_iter = clf_val.n_iter_

    print(f"Retraining Classifier on full data with {clf_best_iter} trees ...")
    final_clf = HistGradientBoostingClassifier(
        max_iter=clf_best_iter,
        learning_rate=0.05,
        max_leaf_nodes=127,
        min_samples_leaf=20,
    ...
    # ── שלב ב': אימון רגרסיה (MAE) רק על ערכים חיוביים ────────────────────────
    mask_pos = y > 0
    X_pos, y_pos = X[mask_pos], y[mask_pos]

    print("Training Regressor (MAE on demand > 0, early stopping) ...")
    reg_val = HistGradientBoostingRegressor(
        loss="absolute_error",
        max_iter=3000,
        learning_rate=0.05,
        max_leaf_nodes=127,
    ...
    reg_val.fit(X_pos, y_pos)
    reg_best_iter = reg_val.n_iter_

    print(f"Retraining Regressor on full data with {reg_best_iter} trees ...")
    final_reg = HistGradientBoostingRegressor(
        loss="absolute_error",
        max_iter=reg_best_iter,
        learning_rate=0.05,
        max_leaf_nodes=127,
        min_samples_leaf=20,
        early_stopping=False,
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--output", default=None, help="Override default output weights path"
    )
