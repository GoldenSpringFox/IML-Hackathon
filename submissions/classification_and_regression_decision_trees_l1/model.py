...
FEATURE_COLS = (
    ...
    + [f"f_{c}" for c in STATION_META_COLS]
    + [
        # historical demand statistics (precomputed from training grid)
        "f_station_mean",
        "f_station_hour_mean",
        "f_station_weekday_hour_mean",
        "f_city_mean",
        "f_city_hour_mean",
        "f_city_weekday_hour_mean",
    ]
)
...
def build_features(df: pd.DataFrame, artifacts: dict) -> np.ndarray:
    ...
    out["f_city"] = (
        city_col.map(artifacts["city_encoder"]).fillna(-1).values.astype(float)
    )
    ...
    c_mean = (
        city_col.map(artifacts["city_stats"]).fillna(global_mean).values.astype(float)
    )
    c_h = (
        (city_col + "_" + hour_s)
        .map(artifacts["city_hour_stats"])
        .fillna(global_mean)
        .values.astype(float)
    )
    c_wh = (
        (city_col + "_" + weekday_s + "_" + hour_s)
        .map(artifacts["city_weekday_hour_stats"])
        .fillna(global_mean)
        .values.astype(float)
    )
    ...
    s_h_raw = (
        (station_key + "_" + hour_s)
        .map(artifacts["station_hour_stats"])
        .values.astype(float)
    )
    s_wh_raw = (
        (station_key + "_" + weekday_s + "_" + hour_s)
        .map(artifacts["station_weekday_hour_stats"])
        .values.astype(float)
    )
    ...
    for col in WEATHER_COLS:
        out[f"f_{col}"] = (
            df[col].values.astype(float) if col in df.columns else np.full(n, np.nan)
        )
...
class BikeDemandModel:
    ...
    def predict(self, test_df: pd.DataFrame) -> np.ndarray:
        ...
        # בניית הפיצ'רים בדיוק כפי שהיה
        X = build_features(test_df, self.artifacts)

        # שליפת שני המודלים מהזיכרון
        clf = self.artifacts["model_clf"]
        reg = self.artifacts["model_reg"]

        # 1. המסווג קובע: 1 אם יש ביקוש, 0 אם אין
        is_positive = clf.predict(X)

        # 2. הרגרסיה חוזה את כמות הביקוש המדויקת
        pred_values = reg.predict(X)

        # 3. הכפלה: כל מקום שהמסווג אמר 0, יאפס את תוצאת הרגרסיה
        final_preds = is_positive * pred_values
